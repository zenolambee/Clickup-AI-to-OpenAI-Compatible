...
from kimichat.client import ChatResult, KimiChatClient
from kimichat.config import Settings, load_account_from_env, load_settings
from kimichat.exceptions import KimiChatError
from kimichat.models import (
    cache_openai_models,
...
async def _stream_openai(
    ...
    try:
        ...
        err = {"error": {"message": str(e), "type": "kimi_error", "code": e.status_code}}
        yield f"data: {json.dumps(err)}\n\n"


async def _ensure_models(client: KimiChatClient) -> None:
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="KimiChat", version="0.1.0")
    ...
    def get_client() -> KimiChatClient:
        account = load_account_from_env(settings)
        return KimiChatClient(account, base_url=settings.base_url)

...
from qwenchat.client import ChatResult, QwenChatClient
from qwenchat.config import Settings, load_account_from_env, load_settings
from qwenchat.exceptions import QwenChatError
from qwenchat.models import (
    cache_openai_models,
...
async def _stream_openai(
    ...
    try:
        ...
        err = {"error": {"message": str(e), "type": "qwen_error", "code": e.status_code}}
        yield f"data: {json.dumps(err)}\n\n"


async def _ensure_models(client: QwenChatClient) -> None:
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="QwenChat", version="0.1.0")
    ...
    def get_client() -> QwenChatClient:
        account = load_account_from_env(settings)
        return QwenChatClient(account, base_url=settings.base_url)

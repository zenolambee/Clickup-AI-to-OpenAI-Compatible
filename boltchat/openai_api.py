...
from boltchat.client import BoltChatClient, ChatResult
from boltchat.config import Settings, load_account_from_env, load_settings
from boltchat.exceptions import BoltChatError
from boltchat.models import (
    cache_openai_models,
...
async def _stream_openai(
    ...
    try:
        ...
        yield "data: [DONE]\n\n"
    except BoltChatError as e:
        err = {"error": {"message": str(e), "type": "bolt_error", "code": e.status_code}}
        yield f"data: {json.dumps(err)}\n\n"


def create_app(settings: Settings | None = None) -> FastAPI:
    settings = settings or load_settings()
    app = FastAPI(title="BoltChat", version="0.1.0")
    ...
    def get_client() -> BoltChatClient:
        account = load_account_from_env(settings)
        return BoltChatClient(account, base_url=settings.base_url, ws_url=settings.ws_url)

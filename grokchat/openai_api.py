...
from grokchat.client import ChatResult, GrokChatClient
from grokchat.config import Settings, load_account_from_env, load_settings
from grokchat.exceptions import GrokChatError
from grokchat.models import (
    cache_openai_models,
...
async def _stream_openai(
    ...
    try:
        stream, chat_id, finalize = await client.stream_deltas(
        ...
        yield "data: [DONE]\n\n"
    except GrokChatError as e:
        err = {"error": {"message": str(e), "type": "grok_error", "code": e.status_code}}
        yield f"data: {json.dumps(err)}\n\n"


async def _ensure_models(client: GrokChatClient) -> None:
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="GrokChat", version="0.1.0")
    ...
    def get_client() -> GrokChatClient:
        account = load_account_from_env(settings)
        return GrokChatClient(account, base_url=settings.base_url)
